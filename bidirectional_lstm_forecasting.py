...
# ----------------------------
base_dirs = ["visualization/bidirectional_lstm", "logs/bidirectional_lstm", "trained_models/bidirectional_lstm"]
...
model = Sequential([
    Bidirectional(LSTM(64, activation='relu'), input_shape=(seq_len, 1)),
    Dense(1)
])
...
history = model.fit(
    ...
    callbacks=[
        CustomLogger(),
        CSVLogger("logs/bidirectional_lstm/train_log.csv", append=False),
...
# ----------------------------
model.save("trained_models/bidirectional_lstm/model.h5")
joblib.dump(sc_X, "trained_models/bidirectional_lstm/scaler_X.pkl")
joblib.dump(sc_y, "trained_models/bidirectional_lstm/scaler_y.pkl")
joblib.dump(le_disease, "trained_models/bidirectional_lstm/label_encoder.pkl")
...
metrics_df.to_csv("visualization/bidirectional_lstm/evaluation_metrics.csv", index=False)

# Line plot for metrics
plt.figure(figsize=(8, 5))
plt.plot(metrics_df['Metric'], metrics_df['Value'], marker='o', linestyle='-', linewidth=2, color='teal')
plt.title("Bidirectional LSTM Evaluation Metrics")
plt.ylabel("Score")
plt.grid(True, linestyle='--', color='gray')
plt.tight_layout()
plt.savefig("visualization/bidirectional_lstm/metrics_plot_line.png")
plt.close()
...
df_out.to_csv("visualization/bidirectional_lstm/forecasted_diseases_next_season.csv", index=False)

# Horizontal bar chart for forecast
plt.figure(figsize=(10, 8))
colors = plt.cm.PuBuGn(np.linspace(0.5, 1, len(df_out)))
plt.barh(df_out['Disease'], df_out['Predicted_Count'], color=colors)
plt.xlabel("Predicted Case Count")
plt.title("Bidirectional LSTM - Forecasted Disease Counts for Next Season", fontsize=13)
plt.grid(True, linestyle='--', axis='x', color='darkgray')
plt.tight_layout()
plt.savefig("visualization/bidirectional_lstm/forecasted_counts_plot_hbar.png")
plt.close()

print("\n=== Forecasted Disease Counts (Next Season - Bidirectional LSTM) ===")
