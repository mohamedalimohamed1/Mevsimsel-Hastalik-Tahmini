...
# ----------------------------
base_dirs = ["visualization/gru", "logs/gru", "trained_models/gru"]
...
history = model.fit(
    ...
    callbacks=[
        CustomLogger(),
        CSVLogger("logs/gru/train_log.csv", append=False),
...
# ----------------------------
model.save("trained_models/gru/model.h5")
joblib.dump(sc_X, "trained_models/gru/scaler_X.pkl")
joblib.dump(sc_y, "trained_models/gru/scaler_y.pkl")
joblib.dump(le_disease, "trained_models/gru/label_encoder.pkl")
...
metrics_df.to_csv("visualization/gru/evaluation_metrics.csv", index=False)

# Pie chart for visual metrics
colors = ['#2F4F4F', '#556B2F', '#8B0000', '#4682B4']
explode = (0.05, 0.05, 0.05, 0.1)

plt.figure(figsize=(7, 7))
plt.pie(metrics_df['Value'], labels=metrics_df['Metric'], explode=explode,
        autopct='%1.1f%%', startangle=140, colors=colors, textprops={'fontsize': 11})
plt.title("GRU Evaluation Metrics (Proportions)", fontsize=14)
plt.tight_layout()
plt.savefig("visualization/gru/metrics_plot_pie.png")
plt.close()
...
df_out.to_csv("visualization/gru/forecasted_diseases_next_season.csv", index=False)

# Horizontal bar chart for forecast
plt.figure(figsize=(10, 8))
bar_colors = plt.cm.Blues(np.linspace(0.4, 1, len(df_out)))
plt.barh(df_out['Disease'], df_out['Predicted_Count'], color=bar_colors)
plt.xlabel("Predicted Case Count")
plt.title("GRU - Forecasted Disease Counts for Next Season", fontsize=13)
plt.grid(True, linestyle='--', axis='x', color='darkgray')
plt.tight_layout()
plt.savefig("visualization/gru/forecasted_counts_plot_hbar.png")
plt.close()

print("\n=== Forecasted Disease Counts (Next Season - GRU) ===")
