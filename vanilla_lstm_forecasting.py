...
# ----------------------------
base_dirs = ["visualization/vanilla_lstm", "logs/vanilla_lstm", "trained_models/vanilla_lstm"]
...
history = model.fit(
    ...
    callbacks=[
        CustomLogger(),
        CSVLogger("logs/vanilla_lstm/train_log.csv", append=False),
...
# ----------------------------
model.save("trained_models/vanilla_lstm/lstm_forecast_model.h5")
joblib.dump(sc_X, "trained_models/vanilla_lstm/scaler_X.pkl")
joblib.dump(sc_y, "trained_models/vanilla_lstm/scaler_y.pkl")
joblib.dump(le_disease, "trained_models/vanilla_lstm/label_encoder.pkl")
...
print("\n=== Evaluation (Per-Disease Forecasting) ===")
print(f"MAE: {mae:.2f}")
...
metrics_df.to_csv("visualization/vanilla_lstm/evaluation_metrics.csv", index=False)

# Plot metrics
plt.figure(figsize=(8, 5))
plt.bar(metrics_df['Metric'], metrics_df['Value'], color='gray')
plt.title("Model Evaluation Metrics")
plt.ylabel("Score")
plt.grid(True)
plt.tight_layout()
plt.savefig("visualization/vanilla_lstm/metrics_plot.png")
plt.close()
...
df_out.to_csv("visualization/vanilla_lstm/forecasted_diseases_next_season.csv", index=False)

# Plot prediction comparison
plt.figure(figsize=(12, 6))
plt.bar(df_out['Disease'], df_out['Predicted_Count'], color='black')
plt.title("Forecasted Disease Counts for Next Season")
plt.ylabel("Predicted Case Count")
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig("visualization/vanilla_lstm/forecasted_counts_plot.png")
plt.close()

print("\n=== Forecasted Disease Counts for Next Season ===")
